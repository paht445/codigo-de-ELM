...
function [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,test_gmean] =...
    ELMregu(entradas_entrenamiento, salidas_entrenamiento,entradas_testeo, salidas_testeo, ...
    Elm_Type, NumberofHiddenNeurons, ActivationFunction,C)
...
OutputWeight=((H*H'+speye(NumberofHiddenNeurons)/C)\(H*T')); %LxL
else
% n = size(T,2);
OutputWeight=H*((H'*H+speye(size(T,2))/C)\(T')); %NxN
end
