...
%%
%esteeeeee essss
salidas_entrenamiento=T2_entrenamiento'+1;
entradas_entrenamiento= datos_entrenamiento;
salidas_testeo=T2_testeo'+1;
entradas_testeo= datos_testeo;

N=1700;%round(linspace(10,2500,30));
PR = 2.^5%(-20:4:20);
%%
%Normalizacion de -1 a 1
entradas_entrenamiento=(entradas_entrenamiento/max(max(entradas_entrenamiento)))*2-1;
entradas_testeo=(entradas_testeo/max(max(entradas_testeo)))*2-1;
%%
...
[TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,test_gmean] = ...
        ELMregu(entradas_entrenamiento, salidas_entrenamiento, entradas_testeo,...
        salidas_testeo, 1, N(a), 'sig', PR(jjj));
...
promediosTest = mean(mean(mean(TestingAccuracyv2, 3)))
promediosTrain = mean(mean(mean(TrainingAccuracyv2, 3)))
meanTrainingTime = mean(mean(mean(TrainingTimev2, 3)))
meanTestingTime = mean(mean(mean(TestingTimev2, 3)))
promediogmeanvv=mean(mean(mean((Testgmeanv2),3)))
%%
% Graficar resultados en contorno
